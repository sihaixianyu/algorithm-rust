...
impl Solution {
    ...
    pub fn swap_pairs_pro(head: Option<Box<ListNode>>) -> Option<Box<ListNode>> {
...
mod tests {
    ...
    fn test_swap_pairs() {
        ...
        let ans = Solution::swap_pairs(list.head);
        assert_eq!(ans, LinkedList::from([2, 1, 4, 3].as_slice()).head);
    }

    #[test]
    fn test_swap_pairs_pro() {
        let list = LinkedList::from([1, 2, 3, 4].as_slice());

        let ans = Solution::swap_pairs_pro(list.head);
        assert_eq!(ans, LinkedList::from([2, 1, 4, 3].as_slice()).head);
    }
}
